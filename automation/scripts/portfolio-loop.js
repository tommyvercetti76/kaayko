...
function buildModelRoster(runtimeConfig) {
  ...
  let installedModels = [];
  try {
    const tags = fetchOllamaTags(runtime, 10000);
    ...
    console.warn(`  ⚠ Ollama model detection failed: ${e.message}`);
...
function buildAutoReview(config, manifest, existingReview, options) {
  ...
  const hardNonPassingGates = nonPassingGates.filter((gate) =>
    gate.status !== "blocked" || gate.blocking_reason !== "environment_restriction"
  );
  const findings = [];
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-link {
    ...
    padding: 8px 16px; text-decoration: none; display: inline-flex;
  ...
  .nav-dot { font-size: 0.6rem; }
  .nav-dot.online { color: var(--green); }
  .nav-dot.offline { color: var(--red); }
...
<div class="header">
  ...
  <div class="header-meta">
    <a href="http://localhost:7799" class="nav-link" id="nav-api" title="Switch to API Dashboard">\u2194 API Dashboard <span class="nav-dot" id="api-dot">\u25cf</span></a>
...
const SIBLING_BASE = 'http://localhost:7799';
...
async function checkSibling(){try{const r=await fetch(SIBLING_BASE+'/api/health',{signal:AbortSignal.timeout(2000),mode:'cors'});const dot=document.getElementById('api-dot');if(dot)dot.className='nav-dot '+(r.ok?'online':'offline')}catch{const dot=document.getElementById('api-dot');if(dot)dot.className='nav-dot offline'}}
...
function handleServeCommand(config, runtimeConfig, args) {
  ...
  try {
    ...
    const dashDir = DASHBOARD_ROOT;
    const maxAgeMs = 7 * 24 * 60 * 60 * 1000;
    const maxSizeBytes = 50 * 1024 * 1024;
    const now = Date.now();
    let pruned = 0;
    for (const [dir, pattern] of [[dashDir, /^live-.*\.log$/], [logsDir, /\.(log|txt)$/]]) {
      try {
        if (!fs.existsSync(dir)) continue;
        for (const f of fs.readdirSync(dir).filter((n) => pattern.test(n))) {
          const fp = path.join(dir, f);
          const stat = fs.statSync(fp);
          if (now - stat.mtimeMs > maxAgeMs || stat.size > maxSizeBytes) { fs.unlinkSync(fp); pruned++; }
        }
      } catch {}
    }
    if (pruned > 0) console.log(`  Pruned ${pruned} old log file(s)`);
  } catch {}
  ...
  const server = http.createServer((req, res) => {
    ...
    // CORS for local dev — restrict to localhost origins only
    const origin = req.headers.origin || "";
    const allowedOrigin = origin.startsWith("http://localhost:") || origin.startsWith("http://127.0.0.1:") ? origin : `http://${host}:${port}`;
...
function buildKnowledgeGraph(config) {
  ...
  Object.entries(profiles).forEach(([id, profile]) => {
    ...
    product.frontend_files = uniqueStrings(product.frontend_files);
    product.backend_files = uniqueStrings(product.backend_files);
