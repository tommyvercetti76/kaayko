...
// The 17 curated spot pages are generated by scripts/generate-spot-pages.js.
// They are appended here from the same source of truth so the sitemap can never
// drift out of sync with the pages that actually exist on disk.
for (const spot of require('./spot-content')) {
  routes.push({
    loc: `/paddlingout/${spot.slug}`,
    source: `src/paddlingout/${spot.slug}.html`,
    changefreq: 'daily',
    priority: '0.9',
  });
}
