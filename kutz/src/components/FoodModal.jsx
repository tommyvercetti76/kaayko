...
/**
 * Add or Edit food modal — includes carbs + fat + micronutrient fields.
...
export default function FoodModal({ defaultMeal = 'snacks', food = null, onAdd, onUpdate, onClose }) {
  ...
  async function submit(e) {
    ...
    try {
      const payload = {
        ...
        iron:     Math.round((Number(form.iron)    || 0) * 10) / 10,
        calcium:  Math.round((Number(form.calcium) || 0) * 10) / 10,
        b12:      Math.round((Number(form.b12)     || 0) * 10) / 10,
        zinc:     Math.round((Number(form.zinc)    || 0) * 10) / 10,
        meal:     form.meal,
      };
      if (isEdit) {
  ...
  const macroFields = [
    ...
    { key: 'fiber',    label: 'Fiber g',    color: '#a78bfa',     placeholder: '3'    },
  ];

  const microFields = [
    { key: 'iron',    label: 'Iron mg',   color: '#f87171', placeholder: '3.0', step: '0.1' },
    { key: 'calcium', label: 'Calcium mg',color: '#fbbf24', placeholder: '180', step: '1'   },
    { key: 'b12',     label: 'B12 mcg',   color: '#60a5fa', placeholder: '0.4', step: '0.1' },
    { key: 'zinc',    label: 'Zinc mg',   color: '#34d399', placeholder: '1.0', step: '0.1' },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: 'rgba(0,0,0,0.7)' }}>
      ...
      >
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          {/* Micronutrient inputs — 2+2 layout, optional */}
          <p className="text-xs" style={{ color: COLORS.textMuted }}>Micronutrients (optional)</p>
          <div className="flex gap-2">
            {microFields.slice(0, 2).map(({ key, label, color, placeholder, step }) => (
              <div key={key} className="flex-1">
                <label className="text-xs block mb-1" style={{ color }}>{label}</label>
                <input
          ...
          <div className="flex gap-2">
            {microFields.slice(2).map(({ key, label, color, placeholder, step }) => (
              <div key={key} className="flex-1">
                <label className="text-xs block mb-1" style={{ color }}>{label}</label>
                <input
                  type="number" inputMode="decimal" placeholder={placeholder} step={step}
